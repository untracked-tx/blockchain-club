...
function DataExportSection({ members, whitelist, isLoading }: DataExportSectionProps) {
  ...
  const exportToCSV = () => {
    ...
    const csvRows = [
      ...
      ...data.map(item => 
        selectedFields.map(field => {
          const value = formatFieldValue((item as any)[field], field)
    ...
    link.setAttribute('download', `blockchain-club-data-${new Date().toISOString().split('T')[0]}.csv`)
    ...
    toast({
      ...
      description: `Downloaded ${data.length} records with ${selectedFields.length} fields`,
      duration: 5000
    })
  }
  ...
  const exportToJSON = () => {
    ...
    const url = URL.createObjectURL(blob)
    link.setAttribute('href', url)
    link.setAttribute('download', `blockchain-club-data-${new Date().toISOString().split('T')[0]}.json`)
    link.style.visibility = 'hidden'
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h4 className="text-sm font-medium">Select Data Fields to Export:</h4>
          ...
          <div key={category} className="border rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              ...
              <Button
                ...
                className="text-xs h-6"
              >
                {availableFields.filter(f => f.category === category).every(f => selectedFields.includes(f.key)) 
                ...
                .map(field => (
                  <label key={field.key} className="flex items-center space-x-2 text-sm">
                    <input
                      ...
                      className="h-3 w-3"
                    />
                    <span className={selectedFields.includes(field.key) ? 'font-medium' : ''}>
                      {field.label}
                    </span>
                  </label>
                ))}

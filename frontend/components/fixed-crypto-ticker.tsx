...
const CryptoTicker: React.FC = () => {
  ...
  const fetchCryptoData = useCallback(async (forceReload = false) => {
    ...
    try {
      ...
      try {
        ...
        setError('Failed to fetch extended coin list. Please try again later.');
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (Math.abs(position) > fullWidth && position < 0) {
      // Use a small offset to make the transition smoother
      setPosition(tickerWidth - 5);
    }
  ...
  return (
    <div className="flex flex-col w-full max-w-full overflow-hidden bg-white text-gray-900 p-4 rounded-lg shadow-lg border border-gray-200">
      <div className="flex justify-between items-center mb-3">
        ...
        <div className="flex items-center space-x-2">          {loading ? (
        ...
        <div className="bg-gray-100 rounded-lg p-3 mb-3 border border-gray-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div>              <label className="text-xs text-gray-600 block mb-1">Number of coins</label>
              <select
                value={coinCount}
            ...
            <div>              <label className="text-xs text-gray-600 block mb-1">Refresh interval</label>
              <select
        ...
        {error && (
        <div className="bg-red-100 text-red-600 p-2 mb-3 rounded text-sm border border-red-300">
          {error}
        </div>
      )}
